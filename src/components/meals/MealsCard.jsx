...
const MealsCard = ({ id, title, image, rating, price }) => {
  return (
    <Card className="shadow-none pt-0 overflow-clip border-none bg-white group justify-between gap-2">
      <CardHeader className="px-0 relative">
        ...
        <div className="absolute top-2 right-2">
            <Badge className={'text-sm xs:text-base bg-accent text-dark'}>{formatCurrency(price || 0)}</Badge>
        </div>
      </CardHeader>
      ...
      <CardFooter className={"px-2 mt-1 justify-between"}>
        <Button asChild className="w-full border-primary text-primary hover:bg-primary hover:text-white" variant="outline" >
